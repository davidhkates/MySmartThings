...
metadata {
	definition(name: "SmarterSense Multi Sensor", namespace: "davidhkates", author: "David Kates", runLocally: true, minHubCoreVersion: '000.017.0012', executeCommandsLocally: false, mnmn: "SmartThings", vid: "generic-contact-2") {
	...
	tiles(scale: 2) {
        ...
        */
		valueTile("temperature", "device.temperature", width: 2, height: 2) {
        	// def decimalValue = currentValue as Double
            // def stringValue = String.format("%.2f", decimalValue)
			state("temperature", label:'${currentValue}°', canChangeIcon: true, unit:"F",
            // state("temperature", label:'$(stringValue)', unit:"F",
					backgroundColors: [
		...
		main(["temperature", "contact", "acceleration"])
...
def parse(String description) {
	...
	if (!maps[0]) {
		...
		map.descriptionText = String.format("%.2f", value)
		*/
